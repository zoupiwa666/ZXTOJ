...
JUDGE_CPU_LIMIT=${JUDGE_CPU_LIMIT:-1}
...
if [ ! -f .env ]; then
  ...
  read -p "沙箱 CPU 限制(核) [默认 1]: " inp
  JUDGE_CPU_LIMIT=${inp:-1}
...
# 等待数据库就绪（用真实认证查询 SELECT 1 探测，ping 只要服务器活着就返回成功不可靠）
info "等待数据库就绪..."
DB_READY=0
DB_AUTH=(-uroot)
for i in $(seq 1 60); do
  if docker exec zxt-db mariadb -uroot -p$DB_PASS -e "SELECT 1" >/dev/null 2>&1; then
    DB_AUTH=(-uroot -p$DB_PASS); DB_READY=1; break
  fi
  if docker exec zxt-db mariadb -uroot -e "SELECT 1" >/dev/null 2>&1; then
    DB_AUTH=(-uroot); DB_READY=1; break
  fi
  sleep 2
done
